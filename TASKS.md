...
- [x] P17-1: WebDAV 客户端封装（PROPFIND/GET/PUT/DELETE，支持 Basic Auth + Token）
- [x] P17-2: 同步引擎（文件 hash + mtime 比对，变更检测，增量推拉）
- [x] P17-3: 插件设置面板（服务器地址、认证方式、同步间隔、同步目录）
- [x] P17-4: 自动同步策略（保存时推送 + 定时轮询拉取 + 启动时全量校验）
- [x] P17-5: 状态栏同步指示器（未同步/同步中/已同步/冲突/离线）
- [x] P17-6: 冲突处理 UI（时间戳对比 + 覆盖本地/保留本地/两者都保留）
